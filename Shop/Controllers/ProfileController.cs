...
namespace Shop.Controllers
{
    public class ProfileController : Controller
    {
        ...
        {
            List<Orden> ordens = DBConnectionAngelo.GetUserOrders(connectionstr, idcliente);
            List<DetalleOrden> detalleOrdenes = new List<DetalleOrden>();
            foreach (Orden orden in ordens)
            {
                decimal subtotal = 0, totalimp = 0, total = 0;
                orden.detalles = DBConnectionAngelo.GetUserOrdersDetails(connectionstr, orden.IDOrden);
